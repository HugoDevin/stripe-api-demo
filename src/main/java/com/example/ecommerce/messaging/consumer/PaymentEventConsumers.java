...
public class PaymentEventConsumers {
  ...
  public void receipt(String body) throws Exception { var m=parse(body); if(done(m,"receipt")) return; Receipt r = new Receipt(); r.orderId=UUID.fromString((String)m.get("orderId")); r.receiptNo="R-"+System.currentTimeMillis(); receiptRepo.save(r); mark(m,"receipt"); }
  ...
  public void accounting(String body) throws Exception { var m=parse(body); if(done(m,"accounting")) return; AccountingEntry e = new AccountingEntry(); e.orderId=UUID.fromString((String)m.get("orderId")); e.debitAccount="Cash"; e.creditAccount="Revenue"; e.amount=new java.math.BigDecimal(m.get("amount").toString()); e.currency=(String)m.get("currency"); accountingRepo.save(e); mark(m,"accounting"); }
